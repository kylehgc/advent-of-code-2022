...
const calculatePrirorityValue = (item) => {
    if (item[0].toUpperCase() === item[0]) {
        return item.charCodeAt(0) - 64 + 26;
    }
    return item.charCodeAt(0) - 96;
};
const getRuckSackDuplicates = (rucksack) => {
...
const getDuplicates = (sackOne, sackTwo) => {
    const duplicateItems = [];
    for (const item of sackOne) {
        if (sackTwo.includes(item)) {
...
const getRuckSackGroupsOf3 = (rucksacks) => {
    const groups = [];
...
const solvePart2 = (rucksacksGroupsOf3) => {
    let total = 0;
    for (const rucksackGroup of rucksacksGroupsOf3) {
        ...
        const duplicates = getDuplicates(duplicatesFirstTwoSacks.join(''), sackThree);
